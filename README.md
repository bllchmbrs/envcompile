...
By default, this decrypts each configured source with its key file, combines the values, writes the encrypted target env file, and writes the generated target private key file outside the repo.
...
Write the compiled env file without encrypting it:

```bash
envcompile compile api --env prod --no-encrypt
```
...
Update `.gitignore` in source, key, and target output directories to ignore private key files, key backups, and generated compiled env files:
...
- With the default encrypted compile, plaintext combined env content is written only to a temporary file, then removed after encryption.
- `compile --no-encrypt` writes plaintext compiled env output and does not generate a target key file.
